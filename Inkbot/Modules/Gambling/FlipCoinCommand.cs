...
namespace NadekoBot.Modules.Gambling
{
    internal class FlipCoinCommand : DiscordCommand
    {
        ...
        public Func<CommandEventArgs, Task> BetFlipCoinFunc() => async e =>
        {
            ...
            bool result = false;
            if (rng.Next(0, 2) == 1) {
                await e.Channel.SendFile("heads.png", Inkbot.Properties.Resources.heads.ToStream(System.Drawing.Imaging.ImageFormat.Png)).ConfigureAwait(false);
                result = true;
            }
            else {
                await e.Channel.SendFile("tails.png", Inkbot.Properties.Resources.tails.ToStream(System.Drawing.Imaging.ImageFormat.Png)).ConfigureAwait(false);
            }
                ...
                if (int.TryParse(e.GetArg("count"), out result))
                {
                    ...
                    for (var i = 0; i < result; i++)
                    {
                        imgs[i] = rng.Next(0, 2) == 0 ?
																		Inkbot.Properties.Resources.tails :
																		Inkbot.Properties.Resources.heads;
                    }
